...
for file in files:
    with open(file) as f:
        content = json.load(f)
        # Correction ici :
        entries = content["results"][models[file.name]]
        # Cherche la clé principale (ex: "glonet", "challenger_model", etc.)
        if isinstance(entries, list):
            for entry in entries:
                model = models.get(file.name, "unknown")
                ...
                # Ajouter le modèle de référence à chaque entrée
